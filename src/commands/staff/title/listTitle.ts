...
import { TitlesQuery, TitlesQueryVariables } from "generated/graphql";
...
const runListTitle = async (message: Message) => {
  ...
  if (data) {
    ...
    const pages = factionList.map(faction => {
...
const buildTitleListPage = (page: any): RichEmbed => {
  const embed = new RichEmbed();
  embed.setColor("GOLD");
  embed.setTitle(`:memo: Liste des titres - ${page.name}`);

  page.titles.forEach((title: any) => {
    let fieldTitle = "";
