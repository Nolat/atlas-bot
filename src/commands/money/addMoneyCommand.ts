...
import { GiveUserMoneyMutation, GiveUserMoneyMutationVariables } from "generated/graphql";
...
const runAddMoney = async (message: Message) => {
  ...
  try {
    ...
    const id = await getParamFromResponse(message,
      `${MONEY_QUESTION}-${id}`,
      WHO_QUESTION,
      60000
    );

    const money = await getParamFromResponse(message,
      `${MONEY_QUESTION}-${Number}`,
      HOW_QUESTION,
      6000
    );
